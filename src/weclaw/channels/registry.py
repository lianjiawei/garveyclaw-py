...
from weclaw.channels.feishu.bot import FeishuBotAdapter, build_event_handler, build_feishu_client
from weclaw.channels.telegram.bot import TelegramMessageSender, build_application, run_polling_options
from weclaw.channels.weixin.bot import WeixinMessageSender, run_weixin_polling
        ...
        while True:
            ...
            try:
                ...
                _print_channel_config_error(
                    ...
                    "Please update TELEGRAM_BOT_TOKEN in .env, or remove it if you only want to use weclaw-tui / Feishu / Weixin.",
                )
                ...
                logger.exception("Bot crashed unexpectedly. Restarting in %s seconds...", TELEGRAM_RESTART_DELAY_SECONDS)
                time.sleep(TELEGRAM_RESTART_DELAY_SECONDS)
            finally:
    ...
    def start(self) -> None:
        while True:
            try:
                ...
                ws_client.start()
                logger.warning("Feishu WebSocket client exited. Restarting in %s seconds...", FEISHU_RESTART_DELAY_SECONDS)
                ...
                if _is_feishu_config_error(exc):
                    _print_channel_config_error(
                        ...
                        "Please update FEISHU_APP_ID / FEISHU_APP_SECRET in .env, or remove them if you only want to use weclaw-tui / Telegram / Weixin.",
                    )
                    logger.error("Feishu runner stopped because startup failed: %s", exc)
                    break
                logger.warning("Feishu runner failed: %s. Restarting in %s seconds...", exc, FEISHU_RESTART_DELAY_SECONDS, exc_info=True)
                time.sleep(FEISHU_RESTART_DELAY_SECONDS)


class WeixinChannelRunner:
    def start(self) -> None:
        while True:
            try:
                print("Weixin bot: iLink long polling started.")
                run_weixin_polling()
                break
            except KeyboardInterrupt:
                print("Bot stopped.")
                ...
                logger.warning("Weixin runner failed: %s. Restarting in %s seconds...", exc, WEIXIN_RESTART_DELAY_SECONDS, exc_info=True)
...
def _has_weixin_config() -> bool:
    return is_effectively_configured(WEIXIN_ACCOUNT_ID) and is_effectively_configured(WEIXIN_TOKEN)
...
def _register_weixin_sender(router: DeliveryRouter) -> None:
    router.register_channel("weixin", WeixinMessageSender())
...
def _build_weixin_runner() -> ChannelStarter:
    return WeixinChannelRunner()
...
def get_registered_channels() -> list[ChannelRegistration]:
    return [
        ...
        ChannelRegistration(
            name="Weixin",
            channel_key="weixin",
            enabled=_has_weixin_config,
            register_sender=_register_weixin_sender,
            start=_build_weixin_runner,
            run_in_background=True,
        ),
    ]
