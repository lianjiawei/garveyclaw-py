...
from weclaw.config import ANTHROPIC_MODEL, OPENAI_MODEL
from weclaw.core.model_profiles import find_profile_by_protocol, get_active_model_profile
...
def get_effective_api_key(provider: str) -> str:
    ...
    if normalized == "claude":
        from weclaw.config import ANTHROPIC_API_KEY

        return ANTHROPIC_API_KEY or ""
    from weclaw.config import OPENAI_API_KEY
...
def get_effective_base_url(provider: str) -> str:
    ...
    if normalized == "claude":
        from weclaw.config import ANTHROPIC_BASE_URL

        return ANTHROPIC_BASE_URL or ""
    from weclaw.config import OPENAI_BASE_URL
