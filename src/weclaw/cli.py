...
PID_FILE = PROJECT_ROOT / "data" / "weclaw.pid"
LOG_FILE = PROJECT_ROOT / "data" / "weclaw.log"
...
def _run_shell_script(name: str) -> int:
    ...
    if not script_path.exists():
        ...
        return 2
    if os.name == "nt":
        print(f"`weclaw {name.removesuffix('.sh')}` 当前用于 Linux / macOS / WSL2 后台运行。")
        print("Windows 原生环境请使用前台模式：`weclaw run`，或在 WSL2 中使用后台命令。")
        return 2
    env = os.environ.copy()
    env.setdefault("WECLAW_PYTHON", sys.executable)
    return subprocess.call(["bash", str(script_path)], cwd=str(PROJECT_ROOT), env=env)


def run_status() -> int:
    values = load_env_values()
    host = values.get("WECLAW_DASHBOARD_HOST") or "127.0.0.1"
    port = values.get("WECLAW_DASHBOARD_PORT") or "8765"
    ...
    if PID_FILE.exists():
        try:
            ...
            print(f"WeClaw PID 文件格式异常：{PID_FILE}")
            return 1
    ...
    print("Run foreground mode: weclaw run")
    return 1
...
def run_logs(lines: int = 80, follow: bool = False) -> int:
    if not LOG_FILE.exists():
        ...
        print("如果你还没有后台启动，请先运行：weclaw start")
        return 1
...
def _print_startup_preflight_error() -> None:
    ...
    print("")
    print("你可以运行 `weclaw setup` 进入初始化向导，或运行 `weclaw doctor` 查看完整诊断。")
    raise SystemExit(2)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv:
        ...
        from weclaw.app import main as run_app
