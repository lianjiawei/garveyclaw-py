...
- `User Interface`：Telegram 和 PowerShell TUI 是两条交互入口，分别面向远程移动端使用和本机终端使用。
- `Bot Layer`：`telegram_bot.py` 负责 Telegram 消息路由、owner 权限判断、命令处理、媒体保存、ASR 转写、异常处理和回复；`tui.py` 负责本地输入循环、命令处理和终端输出。
- `Agent Core`：`claude_client.py` 是智能体编排核心，负责组装 prompt、session、memory、skills、tools，然后通过 Claude Agent SDK 调用模型。
- `Claude Runtime`：Claude Agent SDK 会优先启动 SDK 包内自带的 bundled Claude CLI，再连接兼容 Anthropic API 的模型服务。
- `Media Pipeline`：`media_store.py` 负责图片内存加载和语音保存；`speech_client.py` 通过 `ffmpeg` 和 Vosk 把语音转成文本，再交给 Agent Core。
- `Tools`：`agent_tools.py` 提供自定义 MCP tools，同时白名单允许 Claude Code 内置工具，例如文件读写、搜索和 Bash。
- `State and Knowledge`：`session_store.py`、`memory_store.py` 和本地运行数据共同支持连续会话、长期记忆和对话归档。
- `Skills`：`skill_store.py` 负责加载 Skill prompt，目前主要用于表格分析和数据校验场景。
- `Scheduler Flow`：定时任务从 Telegram 输入创建，写入 `tasks.db`，由 APScheduler 定期检查任务状态和执行时间；到期后通过 `run_agent()` 回到 Agent Core，再把执行结果发回 Telegram。
...
`.env.example` 为了避免暴露本机路径，示例里使用的是相对路径。相对路径会按启动程序时的当前目录解析；推荐始终在项目根目录运行 `garveyclaw` 或 `garveyclaw-tui`。如果你需要从任意目录启动，也可以在自己的 `.env` 中使用本机绝对路径，真实 `.env` 不会提交到 GitHub。
...
第一版 TUI 复用 `AGENT_PROVIDER`、`WORKSPACE_DIR`、memory、skills 和 Agent 工具链；TUI 使用独立会话文件 `data/garveyclaw_session_tui.json`，不会覆盖 Telegram 的连续会话。常用命令：
